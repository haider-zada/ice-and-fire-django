...
class BooksTestCaseV1(APITestCase):
    ...
    def test_create_book(self):
        ...
        response = self.client.post('/api/v1/books/', book, format='json')
    ...
    def test_filter_book_with_name(self):
        body = {'name': 'book test'}

        response = self.client.post('/api/v1/books/filter/', body, format='json')
        self.assertEqual(200, response.data['status_code'])

    def test_filter_book_with_year(self):
        body = {'year': 2019}

        response = self.client.post('/api/v1/books/filter/', body, format='json')
        self.assertEqual(200, response.data['status_code'])

    def test_filter_book_with_name_year(self):
    ...
    def test_update_book(self):
        body = {'name': 'abcd'}
        response = self.client.patch('/api/v1/books/{}/'.format(self.book.id), body, format='json')
        self.assertEqual(200, response.data['status_code'])

    def test_delete_book(self):
        response = self.client.delete('/api/v1/books/{}/'.format(self.book.id), format='json')
